...
// func testValues(t *testing.T) *charts.Values {
...
// func confNsEnabled(t *testing.T) *inject.ResourceConfig {
// 	return inject.
// 		NewResourceConfig(testValues(t), inject.OriginWebhook).
// 		WithNsAnnotations(map[string]string{pkgK8s.ProxyInjectAnnotation: pkgK8s.ProxyInjectEnabled})
// }

// func confNsDisabled(t *testing.T) *inject.ResourceConfig {
// 	return inject.NewResourceConfig(testValues(t), inject.OriginWebhook).WithNsAnnotations(map[string]string{})
// }
...
// 		}
// 		fakeReq := getFakeReq(pod)
// 		conf := confNsEnabled(t).WithKind(fakeReq.Kind.Kind).WithOwnerRetriever(ownerRetrieverFake)
...
// 		conf := confNsDisabled(t).WithKind(fakeReq.Kind.Kind)
// 		patchJSON, err := conf.GetPatch(true)
// 		if err != nil {
// 			t.Fatalf("Unexpected PatchForAdmissionRequest error: %s", err)
// 		}
