...
func loadStoredValuesLegacy(k *k8s.KubernetesAPI) (*charts.Values, error) {
	...
	if idctx.GetTrustDomain() != "" && idctx.GetTrustAnchorsPem() != "" {
		err = fetchIdentityValues(k, idctx, values)
		if err != nil {
			return nil, err
		}
	}
...
// permissions.
func fetchIdentityValues(k kubernetes.Interface, idctx *pb.IdentityContext, values *charts.Values) error {
	if idctx == nil {
		return nil
	}
